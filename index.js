...
// Change Image here:
const foreground = "https://i.postimg.cc/RZy1gn9k/Adobe-Stock-28161076-Preview.jpg";
...
const shaderData = {
    ...
    fragment: `
          ...
          void main(){
              ...
              gl_FragColor = blend(fg, fg);
          }
      `
};

const shaderDataTransparent = {
    uniforms: {
        iResolution: {
            type: "v2",
            value: [window.innerWidth, window.innerHeight]
...
class EffectCanvas {
    ...
    constructor(width, height, loader) {
        ...
        this.background.fillAlphanumber = 1;
        ...
        // this.BackShader = new PIXI.filters.BlurFilter(
        //     "",
        //     shaderData.fragment,

        //     shaderData.uniforms.uTextureForeground
        // );
...
window.onload = () => {
    ...
    function onMouseMove(ev) {
        ...
        const screenWidth = ev.view.screen.width;
        const screenHeight = ev.view.screen.height;

        if (ev.x > ((percentage * screenWidth) + pos.x) || ev.x < (pos.x - (percentage * screenWidth))) {
            displayImage();
            return
        }

        if (ev.y > ((percentage * screenHeight) + pos.y) || ev.y < (pos.y - (percentage * screenHeight))) {
            displayImage();
            return
        }
    }
...
const inserHTMLElements = () => {
    ...
    body.insertAdjacentHTML("afterbegin", `
        ...
        transition: opacity ease 2s, filter ease 2s;
        "></div>
    `)
}